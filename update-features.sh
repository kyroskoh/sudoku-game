#!/bin/bash
# Quick update script for new features

echo "🚀 Updating Sudoku Mastery with new features..."
echo ""
echo "✨ New Features & Fixes:"
echo "  - 🏆 Global Leaderboard System"
echo "  - 👤 Name Entry for Leaderboard"
echo "  - 🎮 Casual Mode Now Tracked on Leaderboard!"
echo "  - 📊 Top 10 Rankings per Mode & Difficulty"
echo "  - 🎯 Fixed Difficulty Selection (Casual & Challenge)"
echo "  - ⬅️ Change Difficulty Button During Gameplay"
echo "  - ⏱️ Complete Time Recording for ALL Modes"
echo "  - ✅ Better Loading States & Feedback"
echo ""

# Push to repository
echo "📤 Pushing to repository..."
git add .
git commit -m "Add leaderboard with name entry and fix difficulty selection" || echo "Nothing to commit"
git push origin main

echo ""
echo "✅ Done! Now deploy on your server:"
echo ""
echo "  ssh sudoku@breezehost-jp"
echo "  cd ~/sudoku-game"
echo "  git pull"
echo "  sudo docker compose down"
echo "  sudo docker compose build --no-cache"
echo "  sudo docker compose up -d"
echo "  sudo docker exec -it sudoku-backend npx prisma migrate deploy"
echo "  sudo docker compose restart"
echo ""
echo "🌐 Then visit:"
echo "  - https://sudoku.kyros.party/casual (complete a puzzle to test name entry)"
echo "  - https://sudoku.kyros.party/leaderboard (see names on leaderboard)"
echo ""
echo "📝 Note: Database migration is required for displayName feature!"

